...
private:
    ...
    void remove(const Comparable &x, BinaryNode *&t) {
        ...
        BinaryNode *parentNode = nullptr;//确保我们找到待删除节点的同时手持其父节点。但若待删除节点为根节点，则为nullptr，这个后续得分类讨论
        while (x != _t->element) {
            parentNode = _t;
            _t = (x < _t->element) ? _t->left : _t->right;
            if (_t == nullptr) {
                //如果搜索时走到了空指针，则说明待删除的元素不存在
                return;
            }
        }
        //现在找到了待删除节点，下面执行删除操作
        BinaryNode *oldNode = _t;
        if (_t->right == nullptr) {
            //如果待删除节点无右子树，则直接删除，并将其左子树连接到父节点上
            if (parentNode != nullptr) {
                //虽然知道父节点，但不知道相对于父节点的左右关系，所以得判断一下再从内存删掉oldNode
                (parentNode->left == oldNode ? parentNode->left : parentNode->right) = _t->left;
            } else {
                //此时待删除节点就是根节点，则要特别地更新根节点，否则打印不了，下同（这个细节卡了我很久）
                root = _t->left;
            }
        } else {
            //如果待删除节点有右子树，则从内存被删除前，会被查找到的右子树最小节点替换
            if (_t->right->left == nullptr) {
                //如果右子树第一个节点就是最小节点，则只用继承待删除节点的左子树
                _t = _t->right;
                _t->left = oldNode->left;
            } else {
                //如果右子树第一个节点不是最小节点，则要找出最小节点，并且继承待删除节点的左、右子树
                _t = detachMin(_t->right);
                _t->left = oldNode->left;
                _t->right = oldNode->right;
            }
            if (parentNode != nullptr) {
                (parentNode->left == oldNode ? parentNode->left : parentNode->right) = _t;
            } else {
                root = _t;
            }
        }
    ...
    /**
     * @brief 提示：查找以t为根的子树中的最小节点，返回这个节点，并从原子树中删除这个节点
     * @param t 子树的根
     * @return 以t为根的子树最小节点
     */
    BinaryNode *detachMin(BinaryNode *&t) {
        BinaryNode *minNode = t;
        BinaryNode *parentNode = nullptr; //确保我们找到待转移的最小节点的同时，手持其父节点，便于转移时更新父节点的left值
        while (minNode->left != nullptr) {
            //detachMin被调用当且仅当待删除节点的右子树第一个节点非最小，所以while循环体至少进行一次，故parentNode将非空
            parentNode = minNode;
            minNode = minNode->left;
        }
        //最小节点无左子树，所以直接让其右子树连接上父节点
